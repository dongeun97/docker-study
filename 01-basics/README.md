...
| | 가상머신 (VM) | 도커 (Container) |
|---|---|---|
| 크기 | 수 GB | 수 MB |
| 시작 속도 | 수 분 | 수 초 |
| OS 포함 | O (OS 전체) | X (호스트 OS 공유) |
| 격리 수준 | 완전 격리 | 프로세스 격리 |
...
동작 과정:
1. 로컬에 `hello-world` 이미지 없음 확인
2. Docker Hub에서 자동으로 이미지 다운로드
3. 다운받은 이미지로 컨테이너 생성
4. 컨테이너 실행 -> `Hello from Docker!` 출력 후 종료
